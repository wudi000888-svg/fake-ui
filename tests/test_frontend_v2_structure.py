...
def test_frontend_v2_module_imports_are_versioned_for_upgrade_cache_busting():
    for path in (ASSETS / "js").rglob("*.js"):
...
def test_tunnel_screen_guides_customers_and_groups_shared_agents_once():
    ...
    assert "function dedicatedAgentCards" in tunnels
    assert "function serviceCard" in tunnels
    assert "function tunnelCard" not in tunnels
    assert "seen.has(key)" in tunnels
    assert "visibleSharedAgentTunnels" in tunnels
