...
同时保留 Hysteria2、用户、套餐、订单、订阅、流量限制、到期时间和一键迁移部署这些基础能力。v3.0.0 新增面向中国本地机器到海外 VPS 的本地服务发布能力：客户只需要把自己的域名解析到 VPS，在面板填写公网域名、本地系统和本地服务端口，即可把本地 `127.0.0.1:8888` 这类端口发布成海外 HTTPS 域名，并自动生成 VPS Xray portal、Nginx HTTPS 入口、Let’s Encrypt 证书，以及 macOS / Linux / Windows 本地客户端 安装包。v3.1.0 新增远程访问：TCP 443 继续给 Nginx、面板、VLESS Reality 和本地服务发布使用，UDP 443 由 Hysteria2 承载 WireGuard 虚拟内网，可用于 Sunshine / Moonlight / RDP / VNC 等跨平台远程桌面场景。
...
| 本地服务发布 | 任意客户域名可映射到 macOS / Linux / Windows 后端本地服务，默认单服务单 Agent，也可高级共享同一本地客户端 |
| 域名资格识别 | 本地服务发布会提示候选/已有域名中已解析到本 VPS 且未被面板、普通节点或其他服务占用的域名；私有 TCP/SSH 明确无需域名 |
| 配对 Agent 包 | 面板生成短期一次性配对 token，本地 Agent 首次运行时主动 bootstrap，成功后清空 token 并写入本地状态 |
| 远程访问 | Hysteria2 UDP 443 承载 WireGuard 虚拟内网，macOS / Linux / Windows 设备可下载通用客户端包，不写死客户域名或机器 |
...
| UDP 443 | Hysteria2 | Hysteria2 节点入口，也承载远程访问的 WireGuard 转发 |
...
| 远程访问 | 管理 macOS / Linux / Windows 设备，生成 Hysteria2 UDP 443 + WireGuard 客户端包，并把 `desktop-*` 账号合并进 Hysteria2 |
...
| `baseline/desktop_*` | 远程访问设备目录、WireGuard/Hysteria2 配置生成、客户端下载包 |
...
本地服务发布底层使用 Xray bridge / portal 完成跨境回连，产品上按“后端机器上的本地端口发布到海外 VPS”理解。它支持两种后端交付方式：

| 方式 | 说明 |
|---|---|
| 配对 Agent 包 | 推荐给普通客户使用。管理员在“本地服务发布”页点击“下载本地客户端”，下载 macOS / Linux / Windows 包；客户运行安装脚本后，Agent 会读取 `agent-profile.json`，调用面板 `/api/agents/bootstrap`，写入 `xray-bridge.json`、`bridge-dashboard.json` 和 `agent-state.json`，然后清空一次性 token |
