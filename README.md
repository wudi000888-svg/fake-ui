...
同时保留 Hysteria2、用户、套餐、订单、订阅、流量限制、到期时间和一键迁移部署这些基础能力。v3.0.0 新增通用内网穿透：客户只需要把自己的域名解析到 VPS，在面板填写公网域名、后端系统和本地服务端口，即可自动生成 VPS Xray portal、Nginx HTTPS 入口、Let’s Encrypt 证书，以及 macOS / Linux / Windows 后端 Agent 安装包。v3.0.1 的后端 Agent 安装包内置仅监听 `127.0.0.1` 的本地可视化控制台，便于客户在自己的机器上查看服务状态、Xray 配置和本地端口连通性。
