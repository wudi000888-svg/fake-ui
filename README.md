...
同时保留 Hysteria2、用户、套餐、订单、订阅、流量限制、到期时间和一键迁移部署这些基础能力。v3.0.0 新增通用内网穿透：客户只需要把自己的域名解析到 VPS，在面板填写公网域名、后端系统和本地服务端口，即可自动生成 VPS Xray portal、Nginx HTTPS 入口、Let’s Encrypt 证书，以及 macOS / Linux / Windows 后端 Agent 安装包。v3.0.1 的后端 Agent 安装包内置仅监听 `127.0.0.1` 的本地可视化控制台；面板还可以生成一次性配对 Agent 包，让客户机器主动向 fake-ui 拉取 Xray bridge 配置，并把同一后端机器的多个服务合并为一份总 JSON/配对包，不需要逐服务复制 JSON。
...
| 域名资格识别 | 内网穿透会提示候选/已有域名中已解析到本 VPS 且未被面板、普通节点或其他穿透占用的域名；私有 TCP/SSH 明确无需域名 |
